...
if (@$_SESSION['email']) {
    if (@$_SESSION['role'] == "admin") {header ("./admin/index.php");}
     elseif (@$_SESSION['role'] == "user") {header ("./pengguna/index.php");}
    
}

//jika tidak ada session/belum login maka buat proses login
//koneksi ke data base
require 'function.php';

// Cek apakah tombol Login ditekan,maka akan mengirim variabel yang ada di form yaitu email & password

if (isset($_POST['Login'])) {

    $email = $_POST['Email']; //Email yang di input oleh user di form
    $userpass = $_POST['Password']; // password yang di input oleh user di form

    // Cek ke database dengan cara query
    $sql = mysqli_query($kon, "SELECT username, password, role FROM user WHERE email='$email'");

    list ($nama, $password, $role) = mysqli_fetch_array($sql); //menampung dalam variabel, ingat harus berurutan sesuai urutan di query

    //Jika data di temukan maka kita verifikasi apakah cocok apa tidak password
    if (mysqli_num_rows($sql) > 0 ){
      /*
        Kita validasi login dengan menggunakan password_verify, karna kita password menggunakan hash
        $userpass ==> diambil dari form login
        $password ==> variabel yang diambil dari password database
      */
            ...
            if (password_verify($userpass, $password)) {
              ...
              session_start();
                ...
                // Jika terverifikasi/berhasil login maka kita arahkan ke halaman index (admin, supervisor, petugas)
                if ($role == "admin") {header("Location: ../admin/index.php");} 
                elseif ($role == "user") {header("Location: ../pengguna/index.php");} 
                
                // exit(); // Hentikan proses selanjutnya setelah pengalihan header
            ...
            echo '<script language="JavaScript">
                    window.alert("LOGIN GAGAL ..! EMAIL/PASSWORD tidak ditemukan");
                    window.location.href="login.php";
                  </script>';
    }
}
...
<!-- Mirrored from prium.github.io/phoenix/v1.13.0/pages/authentication/simple/sign-in.html by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 30 Aug 2023 14:04:00 GMT -->
        ...
        <div class="row flex-center min-vh-100 py-5">
          <div class="col-sm-10 col-md-8 col-lg-5 col-xl-5 col-xxl-3"><a class="d-flex flex-center text-decoration-none mb-4" href="../../../index.html">
            ...
            <div class="mb-3 text-start"><label class="form-label" for="email">Email address</label>
              <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="Email" placeholder="name@example.com" />
            ...
            </div><button class="btn btn-primary w-100 mb-3" type="submit" name="Login">Login</button>
